...
class transport_env():
    ...
    def step(self,action):
        ...
        obs_st = np.zeros(4)
        obs_st[0:2] = self.xt_est[self.cur_time-1,self.obs_locations]
        obs_st[2:4] = self.xt_est[self.cur_time,self.obs_locations]
        next_rp = np.array([next_rp])
        return (obs_st,self.cur_time,next_rp,done)
    ...
    def reset(self):
        ...
        init_reset = np.zeros(5)
        init_reset[0:2] = xt_real[self.start_time,self.obs_locations]
        init_reset[2:4] = xt_real[self.start_time,self.obs_locations]
