...
void LayerGrow(int (&Curr)[IMAGEHEIGHT][IMAGEWIDTH], int (&Prev)[IMAGEHEIGHT][IMAGEWIDTH])
{
    int i,j,k,l;
    int isboundry;
        ...
        for(j=0;j<IMAGEWIDTH;j++){
            if (Curr[i][j] != 0){
                ...
                if (isboundry == 1){
                    for (k=i-10;k<1+10;k++){
                        for (l=j-10;l<j+10;l++){
                            Prev[k][l] = (growkernel[k-10][l-10] || Curr[k][l]);
                        }
                    }
                    isboundry = 0;
                }
...
void LayerSupport(int (&Curr)[IMAGEHEIGHT][IMAGEWIDTH], int (&Prev)[IMAGEHEIGHT][IMAGEWIDTH], int(&Accu)[IMAGEHEIGHT][IMAGEWIDTH])
{
    int tmp,i,j;
    for (i=0;i<IMAGEHEIGHT;i++)
        for(j=0;j<IMAGEWIDTH;j++){
            tmp = (Curr[i][j] || Accu[i][j]) - Prev[i][j];
            if (tmp > 0){
                Prev[i][j] = 1;
            }
            else{
...
void LayerAccu(int (&Curr)[IMAGEHEIGHT][IMAGEWIDTH], int(&Accu)[IMAGEHEIGHT][IMAGEWIDTH])
{
    int i,j;
    for (i=0;i<IMAGEHEIGHT;i++)
        for(j=0;j<IMAGEWIDTH;j++){
            Accu[i][j] = (Curr[i][j] || Accu[i][j]);
        }
}
...
int main(int argc, char* argv[])
{
    ...
    //const char *CLIFileName = "PathPResult.cli";
    const char *CLIFileName = argv[1];
    runningtime.open("result/RunningTime.txt", ios_base::app);
    ofstream result("result/SupportGResult.txt");
    int layer;
    layer = LAYER;
    vector<Data> data_in;
    vector<Data> data_out;
    Data data_tmp;
    int Prev[IMAGEHEIGHT][IMAGEWIDTH];
    int Curr[IMAGEHEIGHT][IMAGEWIDTH];
    int Accu[IMAGEHEIGHT][IMAGEWIDTH];
    int buffer;
    int z,a;
    int i, j, k,l;
    i = 0;
    ...
    pmubarrier[1] = -1;
    pmubarrier[2] = -1;
    string cpu_index[CPU_ITEMS+1] = {"FileIO","Traverse","Total"};
    ...
    for (i=0; i<iNumberOfLayers; i++)
    {
        ...
        if (i==0) {
            ...
            cpu_tmp[0] = get_cpu_time();
            result << "layer " << i << endl;
            cout << "layer " << i << " " << data_in.size() <<endl;
            for (k=0;k<IMAGEHEIGHT;k+=5) {
                for (l=0;l<IMAGEWIDTH;l+=5) {
                    if (Prev[k][l] == 1) 
                        result << (float)k*grid << " " << (float)l*grid << " " << endl;
                }
            }
            result << endl;
            cpu_tmp[1] = get_cpu_time();
    ...
    runningtime << "SupportGeneration 1 runningtime" << endl;
    for (i=0;i<CPU_ITEMS+1;i++) {
